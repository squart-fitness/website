...
class PaymentManager{
	...
	public function getCustomerNameAndNumber($memberID){
		$cust = new Customer;
		$temp = $cust->where(['username' => $memberID, 'gym_id' => auth()->user()->id, 'is_deleted' => 1])->first()->payment->first();
		if(!isset($temp)){
			$username = $cust->select('id', 'name', 'phone', 'fee')->where(['username' => $memberID, 'gym_id' => auth()->user()->id, 'is_deleted' => 1])->first();
			return $username;
		}
		...
		$custEloquent = DB::table('customer_payment')
							...
							->where(['customers.gym_id' => auth()->user()->id, 'customers.username' => $memberID, 'customers.is_deleted' => 1])
	...
	public function addPaymentOfCustomer($data){
 	...
 	public function getPaymentDetailOfAll(){
 		...
 		$payEloquent = DB::table('customer_payment')
							...
							->where(['customers.gym_id' => auth()->user()->id, 'customer_payment.status' => 1, 'customers.is_deleted' => 1])
							->orderByDesc('customer_payment.created_at')
							->get();
 	...
 	public function getPaymentDetailByMonth(int $monthNumber){
 		...
 		$paymentCollection =DB::table('customer_payment') 
 							->join('customers', 'customer_payment.customer_id', '=', 'customers.id')
							->select('customer_payment.*', 'customers.name', 'customers.phone', 'customers.username')
							->where(['customer_payment.gym_id' => auth()->user()->id, 'customer_payment.status' => 1, 'customer_payment.is_deleted' => 1])
