...
class MailManager{
	...
	public function sendRegistrationMail($gymName, $gymEmail, $gymPhone, $customerPhone, $customerEmail, $customerPassword){
		$message = new \stdClass;
                ...
                SendJoiningMail::dispatch($customerEmail, new RegistrationInGym($message))->delay(now()->addSeconds(5));
	}
	...
	public function sendEnquiryMail($gymName, $gymEmail, $gymPhone, $customerEmail){
		$message = new \stdClass;
                ...
                // Mail::to($customerEmail)->send(new GymEnquiry($message));	

                SendJoiningMail::dispatch($customerEmail, new GymEnquiry($message))->delay(now()->addSeconds(5));
	}
}